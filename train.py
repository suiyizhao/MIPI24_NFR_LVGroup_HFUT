...
from models_NAFNet import NAFNet, NAFNetLocal
from utils import *
from options import TrainOptions
from losses import LossLPIPS, LossCont, LossFreqReco
...
scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, 5000, eta_min=1e-7)
...
def train(epoch, optimal):
    ...
    for i, (imgs, gts) in enumerate(train_dataloader):
        ...
        loss = loss_lpips + + opt.lambda_cont * loss_cont + opt.lambda_fft * loss_fft
        ...
        if (i+1) % opt.print_gap == 0:
            print('Training: Epoch[{:0>4}/{:0>4}] Iteration[{:0>4}/{:0>4}] Loss_lpips: {:.4f} Loss_cont: {:.4f} Loss_fft: {:.4f} Time: {:.4f}'.format(epoch, opt.n_epochs, i + 1, max_iter, iter_lpips_meter.average(), iter_cont_meter.average(), iter_fft_meter.average(), iter_timer.timeit()))
...
def val(epoch, optimal):
    ...
    for i, (imgs, gts) in enumerate(val_dataloader):
        ...
        lpips_meter.update(lpips_metric(preds, gts), imgs.shape[0])
        lpips_meter_val.update(lpips_metric(preds_val, gts), imgs.shape[0])
    ...
    print('Epoch[{:0>4}/{:0>4}] LPIPS: {:.4f} LPIPS-Local: {:.4f} Time: {:.4f}'.format(epoch, opt.n_epochs, lpips_meter.average(), lpips_meter_val.average(), timer.timeit())); print('')
    
    if optimal[0] > lpips_meter_val.average():
        optimal[0] = lpips_meter_val.average()
        torch.save(model.state_dict(), models_dir + '/optimal_{:.4f}_{:.4f}_epoch_{:0>4}.pth'.format(optimal[0], lpips_meter.average(), epoch))
        
    writer.add_scalar('lpips', lpips_meter_val.average(), epoch)
